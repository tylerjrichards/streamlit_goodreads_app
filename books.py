    ...
    if has_records:
        ...
        books_chart = (
            (
                ...
                .mark_bar()
                .encode(
                    x=alt.X("index", title="Year"),
                    y=alt.Y("read_at_year", title="Books Read"),
                    tooltip=["index", "read_at_year"],
        ...
        st.altair_chart(books_chart, use_container_width=True)
    else:
...
with row3_2, _lock:
    ...
    pub_df = pd.DataFrame(
        df["book.publication_year"].dropna().value_counts()
    ).reset_index()
    pub_df = pub_df.sort_values(by="index")
    pub_df.columns = ["publication_year", "num"]
    idx = pd.date_range(
        pub_df["publication_year"].min(), pub_df["publication_year"].max(), freq="Y"
    )
    pub_df.index = pd.DatetimeIndex(pub_df["publication_year"])
    pub_df = pub_df.resample("Y").mean().fillna(value=0).reset_index()
    ...
    pub_chart = (
        (
            ...
            .encode(
                ...
                y=alt.Y("num", title="Books Published"),
                color=alt.value("#ADD8E6"),
